...
all_products_list = [860, 861, 863, 864, 865, 866, 867, 868, 869, 870, 872, 873, 874, 875, 876, 877, 
...
def main(callback):
    """
    Open x amount of browser instances equal to the users total cpu thread count + 3
    """
    max_threads = os.cpu_count() + 3 # reduced 4 to 3 to accommodate starting at 0.
    tryout_list = []
    for i in range(len(all_products_list)):
        tryout_list.append(all_products_list[i])
        if i//1 == 1: # Set number of drivers to open at once. Since `i` starts at 0 i've reduced the total max_threads by one.
            drivers = [DriverSetup(f'https://backgroundtown.com/Admin/ProductVariant/Edit/{str(i)}', headless=False)._get_driver() for i in tryout_list]
            callback(drivers)
            [driver.quit() for driver in drivers] # May be faster to not close these drivers and try loggin in again, but instead open a set ammount and keep using those with new urls
            
            
def update_product(temp_drivers):
    with ThreadPoolExecutor(max_workers=os.cpu_count()+4) as executor: # will used the max amount of thread available to the computer, use `max_workers= `to change this
        """
        ...
        """
        executor.map(Login().login, temp_drivers) # log in on all available windows
        executor.map(Variants().sort_sizes, temp_drivers) # hopefully can work after
        # [driver.quit() for driver in temp_drivers]
